...
class QAgent():
    ...
    def update_table(self, history):
        # 한 에피소드에 해당하는 history를 입력으로 받아 q-table의 값을 업데이트한다.
        cum_reward = 0
        for transition in history[::-1]:
            s, a, r, s_prime = transition
            x,y =s
            self.q_table[x,y,a] = self.q_table[x,y,a]+ self.alpha * (cum_reward -\
                self.q_table[x,y,a])
...
def main():
    ...
    for n_epi in range(1000):
        ...
        while not done:
            ...
            s = s_prime
        agent.update_table(history) # update agents using history
        agent.anneal_eps()
